...
export default function Leftcomp() {
    ...
    const refreshBalance = () => {
        setLoading(true);
    axios.post(url + "/user", { id: userAny._id }, { headers: { Authorization: sessionStorage.getItem("token") } })
            .then((res) => {
            ...
            <div className="h-full p-6 m-4 bg-white rounded-2xl shadow-2xl flex flex-col gap-6">
                <div className="bg-cyan-100 rounded-xl py-6 flex flex-col items-center gap-2">
                    <div className="flex flex-col items-center gap-3">
                        <div className="text-5xl font-extrabold text-cyan-700">₹ {userAny.balance ?? 0}</div>
                ...
                <div className="pt-2">
                    ...
                    <div className="pt-4 flex items-center gap-3">
                        ...
                        <div className="bg-blue-100 text-blue-800 text-base font-medium px-3 py-1 rounded dark:bg-gray-700 dark:text-blue-400 border border-blue-400">{userAny.email ?? ""}</div>
                    </div>
                    <div className="pt-4 flex items-center gap-3">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-7 h-7">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z" />
                        </svg>
                        <div className="bg-blue-100 text-blue-800 text-base font-medium px-3 py-1 rounded dark:bg-gray-700 dark:text-blue-400 border border-blue-400">{userAny.number ?? ""}</div>
                    </div>
                    <div className="pt-4 flex items-center gap-3">
                        <img src={uid} alt="UID" className="w-7 h-7" />
                        <div className="bg-blue-100 text-blue-800 text-base font-medium px-3 py-1 rounded dark:bg-gray-700 dark:text-blue-400 border border-blue-400">{userAny.uid ?? ""}</div>
                    </div>
